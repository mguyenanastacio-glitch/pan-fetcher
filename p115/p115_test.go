...
func TestLoginWithProvidedCookies(t *testing.T) {
	cookies := os.Getenv("PANFETCHER_TEST_COOKIES")
	if cookies == "" {
		t.Skip("PANFETCHER_TEST_COOKIES is empty")
	}
	...
	defer func() {
		if agent != nil && agent.StoreInstance != nil {
			_ = agent.StoreInstance.Close()
		}
	}()

	var info elevengo.UserInfo
	if err := agent.Agent.UserGet(&info); err != nil {
		t.Fatalf("UserGet failed: %v", err)
	}
	if info.Id == 0 {
		t.Fatalf("unexpected empty user info: %+v", info)
	}
}
