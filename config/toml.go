...
// SaveProxy updates the proxy.http setting in config.toml.
// If the config file doesn't exist, it creates one with only the proxy setting.
func SaveProxy(proxyHTTP string) error {
	path, found := FindConfigFile()
	if !found {
		// Create a minimal config.toml with proxy setting
		if proxyHTTP == "" {
