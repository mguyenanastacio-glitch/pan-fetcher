...
Rust 版本 [rss2pan](https://github.com/mguyenanastacio/rss2pan) 使用的 Web API 添加离线任务。

移除读取浏览器 cookies 的功能。需要此功能使用 [gcookie](https://github.com/mguyenanastacio/gcookie)
...
REM pan-fetcher 会读取 .cookies 文件
pan-fetcher.exe
```
...
pan-fetcher -q
# 使用cookies
pan-fetcher --cookies "yourcookies"

# 指定 rss URL 离线下载
# 如果 rss.json 存在这条url 的配置，会读取配置。没有配置，默认离线到 115 的默认目录
pan-fetcher -u "https://mikanani.me/RSS/Bangumi?bangumiId=2739&subgroupid=12"
# --no-cache 跳过检查 db.sqlite 里面缓存的
pan-fetcher --no-cache -u "https://mikanani.me/RSS/Bangumi?bangumiId=2739&subgroupid=12"
# --clear-task-type 清除离线任务。 1: 已完成的  2: 所有任务 3: 失败任务 4: 运行的任务 5: 完成并删除的任务 6: 所有的任务
# 清除115任务列表里面已经完成的任务
pan-fetcher --clear-task-type 1

# 查看 magnet 子命令帮助
pan-fetcher magnet -h
pan-fetcher magnet --link "magnet:?xt=urn:btih:12345" --cid "12345" --savepath "文件夹名称"
# 离线包含 magnet 的 txt 文件; 按行分割
pan-fetcher magnet --txt magnet.txt --cid "12345" --savepath "文件夹名称"
# savepath 支持多级路径
pan-fetcher magnet --link "magnet:?xt=urn:btih:12345" --cid "12345" --savepath "文件夹名称/子文件夹名称"
```

### 文件管理命令

`pan-fetcher fs` 提供 115 网盘文件管理命令，可用于搜索整理和目录扁平化。
...
pan-fetcher fs search-mv /anime episode /anime/inbox

# 只移动匹配关键字的 mkv 视频文件
pan-fetcher fs search-mv /anime episode /archive --type 4 --ext mkv

# 将 /video_folder 的后代文件提升到目录本身
pan-fetcher fs flatten /video_folder

# 只查看 flatten 计划，不执行实际移动
pan-fetcher fs flatten /video_folder --dry-run
```

### fs shell

`pan-fetcher fs shell` 提供一个交互式文件管理会话，适合连续执行 `ls`、`cd`、`mv`、`search-mv`、`flatten` 这类操作。

```bash
# 启动交互式 shell
pan-fetcher fs shell

# 调整目录缓存时间
pan-fetcher fs --list-cache-ttl 5s shell

# 关闭目录缓存
pan-fetcher fs --list-cache-ttl 0 shell
```
...
把上面的 batch 例子改成自己的代理地址。另存为 pan-fetcher.cmd 和 pan-fetcher.exe 放在一个目录下面。

在命令行运行 pan-fetcher.cmd 就能够使用自己的代理的了。

<details>
<summary><code><strong>「 点击查看 配置 Linux 定时任务 」</strong></code></summary>
假设 pan-fetcher 目录在 `$HOME` 下面

新建一个 pan-fetcher.sh 的文件

```bash
#!/bin/bash
cd "$(dirname "$0")"
#export HTTPS_PROXY=http://youraddr:port
$HOME/pan-fetcher/pan-fetcher >> $HOME/pan-fetcher/logfile.log 2>&1
```

配置定时任务 `10 8 * * * $HOME/pan-fetcher/pan-fetcher.sh`

不使用 shell 脚本，定时任务这样写 `10 8 * * * cd $HOME/pan-fetcher && ./pan-fetcher`
