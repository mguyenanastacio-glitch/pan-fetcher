...
推送统计 · 订阅活跃度 · 索引器数量（含 Jackett 激活数） · 缓存条目 · 运行时长 · 连接状态 · 最近新增资源（持久化 10 条，重启不丢失）。
...
- **📊 Dashboard** — Push stats, active subs, indexer count, cache entries, uptime, recent items (10, persisted).
