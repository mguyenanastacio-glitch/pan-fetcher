...
`pan-fetcher` 支持统一的 TOML 配置文件和传统的 JSON 配置文件。新用户推荐使用 `config.toml` 统一配置，现有用户可以继续使用原有配置文件，无需修改。
...
2. `~/.config/pan-fetcher/config.toml`
...
```toml
# 如果 config.toml 位于 ~/.config/pan-fetcher/config.toml
[auth]
cookies_file = ".cookies"  # 解析为 ~/.config/pan-fetcher/.cookies

[database]
path = "data/app.db"  # 解析为 ~/.config/pan-fetcher/data/app.db
```
...
1. 当前工作目录的 `rss.json`
2. `~/.config/pan-fetcher/rss.json`

`.cookies`：

1. 当前工作目录的 `.cookies`
2. `~/.config/pan-fetcher/.cookies`

`node-site-config.json`：

1. 当前工作目录的 `node-site-config.json`
2. `~/.config/pan-fetcher/node-site-config.json`
...
```bash
pan-fetcher --rss /path/to/rss.json
```
...
`db.sqlite` 默认优先读取当前工作目录的既有文件；如果当前目录不存在，会继续查找 `~/.config/pan-fetcher/db.sqlite`。两处都不存在时，会在当前工作目录创建新的 `db.sqlite`。
...
```text
/var/lib/pan-fetcher/config.toml
/var/lib/pan-fetcher/.cookies
/var/lib/pan-fetcher/db.sqlite
```

服务的 `WorkingDirectory` 是 `/var/lib/pan-fetcher`，所以这里的 `config.toml` 会被优先读取。脚本生成的基础配置如下：
...
从旧配置迁移到脚本安装目录时，可以先把传统文件放到 `/var/lib/pan-fetcher`，保持兼容读取：

```bash
sudo install -m 600 /path/to/.cookies /var/lib/pan-fetcher/.cookies
sudo install -m 600 /path/to/rss.json /var/lib/pan-fetcher/rss.json
sudo install -m 600 /path/to/node-site-config.json /var/lib/pan-fetcher/node-site-config.json
sudo install -m 600 /path/to/db.sqlite /var/lib/pan-fetcher/db.sqlite
```
...
后续可以继续按下面步骤把 `rss.json` 和 `node-site-config.json` 的内容逐步合并到 `/var/lib/pan-fetcher/config.toml`。

### 步骤 1：创建基础 config.toml

普通命令行或手动部署场景，创建 `~/.config/pan-fetcher/config.toml`，先配置认证部分：

如果使用 Linux 安装脚本，则编辑 `/var/lib/pan-fetcher/config.toml`，不需要再创建 `~/.config/pan-fetcher/config.toml`。
...
```bash
# 使用命令行参数覆盖配置文件
pan-fetcher --port 9000 --cookies "UID=xxx; CID=xxx; SEID=xxx; KID=xxx"
```
